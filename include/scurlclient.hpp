...
public:
  ...
  Response() = default;
  Response(
      std::shared_ptr<CurlHolder> curl,
      std::string&& p_text,
      std::string&& p_header_string,
      Cookies&& p_cookies,
      Error&& p_error)
    : curl_(std::move(curl))
    , text(std::move(p_text))
    , cookies(std::move(p_cookies))
    ...
    curl_easy_getinfo(
        curl_->handle, CURLINFO_SIZE_DOWNLOAD_T, &downloaded_bytes);
    curl_easy_getinfo(curl_->handle, CURLINFO_SIZE_UPLOAD_T, &uploaded_bytes);
#else
    double downloaded_bytes_double, uploaded_bytes_double;
    curl_easy_getinfo(
        curl_->handle, CURLINFO_SIZE_DOWNLOAD, &downloaded_bytes_double);
    curl_easy_getinfo(
        curl_->handle, CURLINFO_SIZE_UPLOAD, &uploaded_bytes_double);
...
private:
   ...
   * Ensures that the "Transfer-Encoding" is set to "chunked", if not overriden
   *in header_.
   **/
...
inline void SCurlSession::SetHeaderInternal() {
  ...
  if (chunkedTransferEncoding &&
      header_.find("Transfer-Encoding") == header_.end()) {
...
inline void SCurlSession::SetConnectTimeout(const ConnectTimeout& timeout) {
  curl_easy_setopt(
      curl_->handle, CURLOPT_CONNECTTIMEOUT_MS, timeout.Milliseconds());
}
...
inline void SCurlSession::SetMultipart(Multipart&& multipart) {
  ...
  for (const Part& part : multipart.parts) {
    ...
    curl_formadd(
        &formpost, &lastptr, CURLFORM_ARRAY, formdata.data(), CURLFORM_END);
  }
...
inline void SCurlSession::SetLimitRate(const LimitRate& limit_rate) {
  curl_easy_setopt(
      curl_->handle, CURLOPT_MAX_RECV_SPEED_LARGE, limit_rate.downrate);
  curl_easy_setopt(
      curl_->handle, CURLOPT_MAX_SEND_SPEED_LARGE, limit_rate.uprate);
}
...
inline void SCurlSession::SetReadCallback(const ReadCallback& read) {
  ...
  curl_easy_setopt(
      curl_->handle, CURLOPT_READFUNCTION, bongo::util::readUserFunction);
...
inline void SCurlSession::SetHeaderCallback(const HeaderCallback& header) {
  curl_easy_setopt(
      curl_->handle, CURLOPT_HEADERFUNCTION, bongo::util::headerUserFunction);
  headercb_ = header;
...
inline void SCurlSession::SetWriteCallback(const WriteCallback& write) {
  curl_easy_setopt(
      curl_->handle, CURLOPT_WRITEFUNCTION, bongo::util::writeUserFunction);
  writecb_ = write;
  curl_easy_setopt(curl_->handle, CURLOPT_WRITEDATA, &writecb_);
}

inline void
SCurlSession::SetProgressCallback(const ProgressCallback& progress) {
  progresscb_ = progress;
#if LIBCURL_VERSION_NUM < 0x072000
  curl_easy_setopt(
      curl_->handle,
      CURLOPT_PROGRESSFUNCTION,
      bongo::util::progressUserFunction);
...
inline void SCurlSession::SetDebugCallback(const DebugCallback& debug) {
  curl_easy_setopt(
      curl_->handle, CURLOPT_DEBUGFUNCTION, bongo::util::debugUserFunction);
  debugcb_ = debug;
...
inline void SCurlSession::SetUnixSocket(const UnixSocket& unix_socket) {
  curl_easy_setopt(
      ...
      unix_socket.GetUnixSocketString());
}
...
inline void SCurlSession::SetSslOptions(const SslOptions& options) {
  if (!options.cert_file.empty()) {
    ...
    if (!options.cert_type.empty()) {
      curl_easy_setopt(
          curl_->handle, CURLOPT_SSLCERTTYPE, options.cert_type.c_str());
    }
  }
  if (!options.key_file.empty()) {
    ...
    if (!options.key_type.empty()) {
      curl_easy_setopt(
          curl_->handle, CURLOPT_SSLKEYTYPE, options.key_type.c_str());
    }
    if (!options.key_pass.empty()) {
      curl_easy_setopt(
          curl_->handle, CURLOPT_KEYPASSWD, options.key_pass.c_str());
    }
  }
#if SUPPORT_ALPN
  curl_easy_setopt(
      curl_->handle, CURLOPT_SSL_ENABLE_ALPN, options.enable_alpn ? ON : OFF);
#endif
#if SUPPORT_NPN
  curl_easy_setopt(
      curl_->handle, CURLOPT_SSL_ENABLE_NPN, options.enable_npn ? ON : OFF);
#endif
  curl_easy_setopt(
      curl_->handle, CURLOPT_SSL_VERIFYPEER, options.verify_peer ? ON : OFF);
  curl_easy_setopt(
      curl_->handle, CURLOPT_SSL_VERIFYHOST, options.verify_host ? 2L : 0L);
#if LIBCURL_VERSION_NUM >= 0x072900
  curl_easy_setopt(
      curl_->handle,
      CURLOPT_SSL_VERIFYSTATUS,
      options.verify_status ? ON : OFF);
#endif
  curl_easy_setopt(
      curl_->handle,
      CURLOPT_SSLVERSION,
  ...
  if (!options.ciphers.empty()) {
    curl_easy_setopt(
        curl_->handle, CURLOPT_SSL_CIPHER_LIST, options.ciphers.c_str());
  }
#if SUPPORT_TLSv13_CIPHERS
  if (!options.tls13_ciphers.empty()) {
    curl_easy_setopt(
        curl_->handle, CURLOPT_TLS13_CIPHERS, options.ciphers.c_str());
  }
#endif
...
inline Response SCurlSession::Download(std::ofstream& file) {
  ...
  curl_easy_setopt(curl_->handle, CURLOPT_CUSTOMREQUEST, "GET");
  curl_easy_setopt(
      curl_->handle, CURLOPT_WRITEFUNCTION, bongo::util::writeFileFunction);
...
inline Response SCurlSession::Get() {
  // In case there is a body or payload for this request, we create a custom
  // GET-Request since a GET-Request with body is based on the HTTP RFC **not**
  // a leagal request.
...
inline Response SCurlSession::Post() {
  ...
  if (hasBodyOrPayload_) {
    ...
    curl_easy_setopt(
        curl_->handle, CURLOPT_POSTFIELDS, readcb_.callback ? nullptr : "");
...
inline Response SCurlSession::makeDownloadRequest() {
  ...
  if (headercb_.callback) {
    ...
    curl_easy_setopt(
        curl_->handle, CURLOPT_HEADERFUNCTION, bongo::util::writeFunction);
...
#  endif
  ...
  if (!this->writecb_.callback) {
    curl_easy_setopt(
        curl_->handle, CURLOPT_WRITEFUNCTION, bongo::util::writeFunction);
