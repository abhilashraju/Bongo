...
inline HttpHeader parseHeader(
    const std::string& headers, std::string* status_line, std::string* reason) {
  HttpHeader header;
  ...
  for (std::string& line : lines) {
    ...
    if (line.length() > 0) {
      ...
      if (found != std::string::npos) {
        ...
        value.resize(std::min<size_t>(
            value.size(), value.find_last_not_of("\t\n\r ") + 1));
...
split(const std::string& to_split, char delimiter) {
...
inline size_t readUserFunction(
    char* ptr, size_t size, size_t nitems, const ReadCallback* read) {
  size *= nitems;
  return read->callback(ptr, size) ? size : CURL_READFUNC_ABORT;
}

inline size_t headerUserFunction(
    char* ptr, size_t size, size_t nmemb, const HeaderCallback* header) {
  size *= nmemb;
  return header->callback({ptr, size}) ? size : 0;
}

inline size_t
writeFunction(char* ptr, size_t size, size_t nmemb, std::string* data) {
  size *= nmemb;
...
inline size_t
writeFileFunction(char* ptr, size_t size, size_t nmemb, std::ofstream* file) {
  size *= nmemb;
  file->write(ptr, size);
  return size;
}

inline size_t writeUserFunction(
    char* ptr, size_t size, size_t nmemb, const WriteCallback* write) {
  size *= nmemb;
  return write->callback({ptr, size}) ? size : 0;
}

#if LIBCURL_VERSION_NUM < 0x072000
inline int progressUserFunction(
 ...
 * If you plan to use this methode on a regular basis think about creating a
 *CurlHolder object and calling urlEncode(std::string) on it.
 *
 * Example:
 * CurlHolder holder;
...
/**
 * Creates a temporary CurlHolder object and uses it to unescape the given
 *string. If you plan to use this methode on a regular basis think about
 *creating a CurlHolder object and calling urlDecode(std::string) on it.
 *
 *
 * Example:
 * CurlHolder holder;
